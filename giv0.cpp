...
int giv1(int n, int nrootx, int njx, double *a, double *root,
	    double *vect[])
{
  int
    nroot,
  ...
  if(!nrootx){			/* possibility nroot == 0 accepted as of */
  ...
  if(n == 1){
    ...
    else if(!nrootx){			// condition root[1] < cut_off
					// eliminated on 21 April 2008
      vect[1][1] = 1.0;
      return 1;
    }
    else{
  ...
  if(!nrootx){			//  cut_off introduced August 12, 1993
    ret = nroot = number_below(cut_off, root, n);
    switch(ret){
    case 0:
      nrootx = ret = nroot = 1;
      // alternative for ret == 0 changed from nroot =0, nrootx = -1 to
      // ret=nrootx=nroot=1 on April 21, 2008 to guarantee that
      // at least one eigenvector be computed
      break;
